...
def find_current_qld_pdf_url(year):
    ...
    try:
        ...
        cached = None
    if cached:
        print(f"[qld] using cached last-known-good URL: {cached}", file=sys.stderr)
        return cached, False
...
def parse_qld_calendar(year, pdf_url=None, pdf_bytes=None):
    ...
    try:
        ...
        if pdf_bytes is None:
            ...
            if not discovered:
                if year != dt.date.today().year:
                    # No calendar published for a non-current year (e.g. a past
                    # year, or one not yet released) is EXPECTED, not a failure.
                    # The fallback PDF is for the current year, so its filename
                    # year-guards out and this pass yields 0 QLD events — fine.
                    print(f"[qld] no {year} calendar published (expected for "
                          f"non-current year); this pass yields no QLD events",
                          file=sys.stderr)
                else:
                    print(f"[qld] WARNING: discovery failed for current year "
                          f"{year}; using pinned FALLBACK PDF (may be STALE): "
        ...
        if not events:
            ...
            # last-known-good fallback so a future run where discovery fails uses
            # it instead of the older hardcoded pin.
            try:
                import pdf_cache
                pdf_cache.save_url("qld", year, resolved_url)
            except Exception:
                pass
