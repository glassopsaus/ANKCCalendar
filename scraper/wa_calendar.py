...
WA_FALLBACK_PDF = ("https://dogswest.com/dogswest/d/Members/Yearly_Show_Date_Calendars/"
                   "D6SXD0IL1XES52V1IFEE8Q2GKBW80F/I8EMOSER8HQSU97.pdf/"
                   "2026+Calendar+Of+Events+220726.pdf")
...
def find_current_wa_pdf_url(year):
    ...
    try:
        import pdf_cache
        cached = pdf_cache.get_cached_url("wa", year)
    except Exception:
        cached = None
    if cached:
        print(f"[wa] using cached last-known-good URL: {cached}", file=sys.stderr)
        return cached, False
...
def parse_wa_calendar(year, pdf_url=None, pdf_bytes=None):
    ...
    try:
        ...
        if pdf_bytes is None:
            ...
            if not discovered:
                if year != dt.date.today().year:
                    print(f"[wa] no {year} calendar published (expected for "
                          f"non-current year); this pass yields no WA events",
                          file=sys.stderr)
                else:
                    print(f"[wa] WARNING: discovery failed for current year "
        ...
        if not events:
            ...
            try:
                import pdf_cache
                pdf_cache.save_url("wa", year, resolved_url)
            except Exception:
                pass
