...
def find_current_dv_pdf_url(year):
    ...
    try:
        import pdf_cache
        cached = pdf_cache.get_cached_url("dv", year)
    except Exception:
        cached = None
    if cached:
        print(f"[dv] using cached last-known-good URL: {cached}", file=sys.stderr)
        return cached, False
...
def parse_dv_calendar(year, pdf_url=None, pdf_bytes=None):
    ...
    try:
        ...
        if pdf_bytes is None:
            ...
            if not discovered:
                if year != dt.date.today().year:
                    print(f"[dv] no {year} calendar published (expected for "
                          f"non-current year); this pass yields no DV events",
                          file=sys.stderr)
                else:
                    print(f"[dv] WARNING: discovery failed for current year "
        ...
        if not events:
            ...
            try:
                import pdf_cache
                pdf_cache.save_url("dv", year, resolved_url)
            except Exception:
                pass
