...
def scrape_show_manager(year, months=range(1, 13), fetch_details=None):
    ...
    if fetch_details:
        ...
        targets = [x for x in listings
                   ...
                   and _upcoming(x)]
        print(f"[sm-detail] fetching details for {len(targets)} upcoming events "
              f"(this adds ~{len(targets)} requests)...", file=sys.stderr)
        n_addr = 0
        for i, x in enumerate(targets, 1):
            ...
            if i % 100 == 0:
                print(f"[sm-detail]   {i}/{len(targets)} done", file=sys.stderr)
        print(f"[sm-detail] got address for {n_addr}/{len(targets)} events",
              file=sys.stderr)
